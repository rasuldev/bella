...
?>
	...
	<form role="form" method="post" action="<?=$arResult["FORM_ACTION"]?>" onsubmit="BX('bx_subscribe_btn_<?=$buttonId?>').disabled=true;">
		...
		<p class="clear emal_podpiska">
			<input class="bx-form-control" type="email" name="SENDER_SUBSCRIBE_EMAIL" value="<?=$arResult["EMAIL"]?>" title="<?=GetMessage("subscr_form_email_title")?>" placeholder="<?=htmlspecialcharsbx(GetMessage('subscr_form_email_title'))?>" style="background-color: #212121; border: 0px; padding-left: 10px"/>
			<input type="submit" value="" id="bx_subscribe_btn_<?=$buttonId?>"/>
			<i class="ico_msg"></i>
		</p>
	</form>
<?
