...
<div id="<? echo $arItemIDs['ID']; ?>">
    ...
    ?>
    <div class="slider_product" id="<? echo $arItemIDs['BIG_SLIDER_ID']; ?>">
        <div id="ax_slider">
            <a href="#" class="ax_slide_img group1">
            ...
            if ($arResult['SHOW_SLIDER']) {
                if (!isset($arResult['OFFERS']) || empty($arResult['OFFERS'])) {
                    ...
                    <div class="<? echo $strClass; ?>" id="<? echo $arItemIDs['SLIDER_CONT_ID']; ?>">
                        <ul class="clear" style="width: 960px;">
                            <?
                    ...
                    <a class="ax_nav_left" href="javascript:void(0);"><i class="ico_left"></i></a>
                    <a class="ax_nav_right" href="javascript:void(0);"><i class="ico_right"></i></a>
                    <?
                } else {
    ...
    <div class="details_prod">
        ...
        <p class="price">
            ...
            if ($arParams['SHOW_OLD_PRICE'] == 'Y') {
                ?>
                <span class="old_price" id="<? echo $arItemIDs['OLD_PRICE']; ?>"
                      style="display: <? echo($boolDiscountShow ? '' : 'none'); ?>"><? echo($boolDiscountShow ? $minPrice['PRINT_VALUE'] : ''); ?></span>
                <?
            }
            ?>
            <span class="now_price"
                  id="<? echo $arItemIDs['PRICE']; ?>"><? echo $minPrice['PRINT_DISCOUNT_VALUE']; ?></span>
            <?
    ...
    ?>
        ...
        if (false)//'' != $arResult['PREVIEW_TEXT']
        {
            if (
        ...
        if (false && isset($arResult['OFFERS']) && !empty($arResult['OFFERS']) && !empty($arResult['OFFERS_PROP'])) {
            $arSkuProps = array();
            ?>
        ...
        <div class="add_in_cart clear">
            ...
            <ul class="price_plus_minus clear">
                <li><a id="<? echo $arItemIDs['QUANTITY_DOWN']; ?>" class="btn_minus" href="javascript:void(0)">-</a>
                </li>

                <li>
                    <input id="<? echo $arItemIDs['QUANTITY']; ?>" class="count_prod" name=""
                ...
                <li><a id="<? echo $arItemIDs['QUANTITY_UP']; ?>" class="btn_plus" href="javascript:void(0)">+</a></li>
            </ul>
            <input class="promo_kod" placeholder="Введите промокод" type="text">
        </div>

		<span class="item_buttons_counter_block" id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>"
              style="display: <? echo($canBuy ? '' : 'none'); ?>;">
    <?
    if ($showBuyBtn)//$showBuyBtn
    {
        ?>
        <a href="javascript:void(0);" class="btn_add_cart"
           id="<? echo $arItemIDs['BUY_LINK']; ?>"><i class="ico_cart"></i><? echo $buyBtnMessage; ?></a>
        <?
    }
    if ($showAddBtn)//$showAddBtn
    {
        ?>
        <a href="javascript:void(0);" class="btn_add_cart"
           id="<? echo $arItemIDs['ADD_BASKET_LINK']; ?>"><i class="ico_cart"></i><? echo $addToBasketBtnMessage; ?></a>
        <?
    }
    ?>
        </span>
        <span id="<? echo $arItemIDs['NOT_AVAILABLE_MESS']; ?>" class="bx_notavailable" style="display: <? echo(!$canBuy ? '' : 'none'); ?>;"><? echo $notAvailableMessage; ?>
        </span>
        <?
        ...
        ?>
        <p class="dop_cat">
            <span>Категория:</span> <a href="<?='http://'.$_SERVER["SERVER_NAME"].'/catalog/'.$arResult["SECTION"]["CODE"].'/'?>"><?=$arResult["SECTION"]["NAME"]?></a>
        </p>
    ...
    <div class="prod_tabs">
        ...
        <div id="tab_oplat">
            ...
            <div>
                ...
                <p>Стоимость доставки в пределах МКАД и до 20 км за МКАД составляет 400 рублей. В пределы МКАД также входят районы Бутово, Жулебино, Новокосино, Реутов, Старое Косино.
                    Доставка осуществляется на следующий день после покупки. Оформляется только на товар, имеющийся в наличии на складах Компании и при условии поступления заказа до 16:00 текущего дня. Время доставки с 9:00 до 15:00 либо с 15:00 до 21:00.
                    Доставка c 20.00 до 00.00 осуществляется через день со дня покупки.
                    Стоимость всех указанных видов доставки одинаковая.
                    Оформив заказ, Вы можете отложить доставку на срок до 14 дней, предупредив об этом оператора.<br />
                    Внимание!!!<br />
                    Если Вам не привезли заказанную в интернет магазине технику в срок в соответствии с указанной датой при создании заказа , то повторная доставка будет для Вас бесплатной*
                </p>
                <p>Оплата производится: наличными при получении, перечеслением на карту Сбербанка, оплата выставленного счета.. </p>
            </div>
