...
<?	if(is_array($arParams["PROPERTY_FIELDS"])
		&& in_array("FEEDBACK_TEXT", $arParams["PROPERTY_FIELDS"]))
	{
		...
		$strFBtext .= '</div>
			...
			<p><textarea cols="" rows="" id="EMPTY_TEXT'.$ALX.'" name="FEEDBACK_TEXT_'.$ALX.'">'.$arResult["FEEDBACK_TEXT"].'</textarea></p></div>';
		if(in_array("FEEDBACK_TEXT_".$ALX, $errorField, true))
		{
			$strFBtext .= '</div>';
		}
...
<?	foreach($arResult["FIELDS"] as $key=>$arField):?>
		...
		<div class="alx_feed_back_form_item_pole">
			<div class="alx_feed_back_form_name">
				<?if($arField["REQUIRED"]):?><span class="alx_feed_back_form_required_text"></span><?endif;
