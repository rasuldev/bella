...
$placeholder = [
	'FIO_FID1' => 'Ваше имя...',
	'PHONE_FID1' => 'Номер телефона',
	'EMAIL_FID1' => 'Ваш E-mail',
	'FID1' => 'Ваше сообщение'
];
...
<form id="f_feedback_<?=$ALX?>" name="f_feedback_<?=$ALX?>" action="<?=$actionPage?>" method="post">
	<h2>Контакты</h2>
...
/*		if(in_array("FEEDBACK_TEXT_".$ALX, $arParams["PROPERTY_FIELDS_REQUIRED"]))
		...
		$strFBtext .= '</div>
			<p class="alx_feed_back_form_inputtext_bg" id="error_EMPTY_TEXT"><textarea cols="" rows="" id="EMPTY_TEXT'.$ALX.'" placeholder="'.$placeholder[$ALX].'" name="FEEDBACK_TEXT_'.$ALX.'">'.$arResult["FEEDBACK_TEXT"].'</textarea><i class="ico_plane"></i></p>';
		if(in_array("FEEDBACK_TEXT_".$ALX, $errorField, true))
		{
			$strFBtext .= '</div>';
		}
...
<?		elseif($arField["TYPE"] != "F"):?>
			<div class="alx_feed_back_form_inputtext_bg" id="error_<?=$arField["CODE"]?>">
<?				if(!empty($_POST["FIELDS"][$arField["CODE"]])):?>
					<p><input type="text" placeholder="<?=$placeholder[$arField["CODE"]] ?>" id="<?=$arField["CODE"]?>" name="FIELDS[<?=$arField["CODE"]?>]" value="<?=trim(htmlspecialcharsEx($_POST["FIELDS"][$arField["CODE"]]))?>" class="alx_feed_back_form_inputtext" /><i class="ico_<?=$arField["CODE"]?>"></i></p>
<?				elseif(!empty($arField["AUTOCOMPLETE_VALUE"])):
					$readonly = "";
...
?>
					<p><input type="text" id="<?=$arField["CODE"]?>" placeholder="<?=$placeholder[$arField["CODE"]] ?>"  name="FIELDS[<?=$arField["CODE"]?>]" <?=$readonly?>value="<?=trim(htmlspecialcharsEx($arField["AUTOCOMPLETE_VALUE"]));?>" class="alx_feed_back_form_inputtext"/><i class="ico_<?=$arField["CODE"]?>"></i></p>
...
<?				else:?>
					<p><input type="text"
						...
						placeholder="<?=$placeholder[$arField["CODE"]] ?>"
						value="<?=$arField["DEFAULT_VALUE"]?>"
						class="alx_feed_back_form_inputtext"
						onblur="if(this.value==''){this.value='<?=$arField["DEFAULT_VALUE"]?>'}" onclick="if(this.value=='<?=$arField["DEFAULT_VALUE"]?>'){this.value=''}"><i class="ico_<?=$arField["CODE"]?>"></i></p>
					<!--
