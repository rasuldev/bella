...
while ($rate = $rateIterator->Fetch())
{
	...
	$currencyCode = (isset($currencyList[$rate['CURRENCY']]) ? htmlspecialcharsex($currencyList[$rate['CURRENCY']]) : $rate['CURRENCY']);
	$row->AddViewField('CURRENCY', '<a href="/bitrix/admin/currency_edit.php?ID='.$rate['CURRENCY'].'&lang='.LANGUAGE_ID.'" title="'.GetMessage('CURRENCY_A_EDIT_TITLE').'">'.$currencyCode.'</a>');
	unset($currencyCode);
